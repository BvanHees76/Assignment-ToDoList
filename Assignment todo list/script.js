...
let makeCompleteListItem = function (e) {
   ...
   //DeLete
   let deleteCompleteListItem = function()
   {
    let selBinButton = document.getElementsByClassName('remove-bin');
    let parentDiv = selBinButton.parentNode;
    parentDiv.remove();

   };

   let deleteFromDom = function()
   {
    let selBinButton = document.getElementsByClassName('remove-bin');
    selBinButton.addEventListener('click', deleteCompleteListItem);
